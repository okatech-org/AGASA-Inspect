...
function getGatewaySecret(flux: FluxCode): string {
  ...
  if (!secret) {
    throw new Error(`Secret HMAC manquant pour ${flux}`);
  }
  return secret;
}
...
export const pushToCore = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const flux: FluxCode = "F3";
    const endpoint = `${getCoreGatewayUrl()}/api/gateway/f3`;
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          await ctx.runMutation(internalApi.sync.outbound.markDispatchSuccess, {
    ...
    await ctx.runMutation(internalApi.sync.outbound.markDispatchError, {
      fluxRefId: args.fluxRefId,
      typeMessage: args.typeMessage,
      payload: JSON.stringify(args.payload),
      attempt: 3,
    ...
    if (args.fluxRefId) {
      ...
      if (existing) {
        ...
        await ctx.db.insert("signaux", {
          type: SIGNAL_TYPES.FLUX_F3_ENVOYE,
  ...
  handler: async (ctx, args) => {
    ...
    if (args.fluxRefId) {
      ...
      if (existing) {
        ...
        await ctx.db.insert("signaux", {
          type: SIGNAL_TYPES.FLUX_F3_ERREUR,
          source: CORTEX.GATEWAY,
          destination: CORTEX.HIPPOCAMPE,
          entiteType: "fluxInterApps",
          entiteId: String(args.fluxRefId),
...
export const listOutboundFlux = query({
  ...
  handler: async (ctx) => {
    ...
    return flux
      .filter((item) => item.fluxCode === "F3" && item.direction === "envoi")
